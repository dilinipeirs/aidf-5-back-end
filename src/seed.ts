...
// IMPORTANT: You need to create this new entity file (e.g., Amenity.ts)
import Amenity from "./infrastructure/entities/Amenity"; 
import { generateEmbedding } from "./application/utils/embeddings";
import stripe from "./infrastructure/stripe";

// A larger set of image URLs to use for the multiple images per hotel
const ALL_IMAGES = [
  "https://cf.bstatic.com/xdata/images/hotel/max1280x900/297840629.jpg?k=d20e005d5404a7bea91cb5fe624842f72b27867139c5d65700ab7f69396026ce&o=&hp=1", // Original IMAGE_1
  "https://cf.bstatic.com/xdata/images/hotel/max1280x900/596257607.jpg?k=0b513d8fca0734c02a83d558cbad7f792ef3ac900fd42c7d783f31ab94b4062c&o=&hp=1", // Original IMAGE_2
  "https://cf.bstatic.com/xdata/images/hotel/max1280x900/308797093.jpg?k=3a35a30f15d40ced28afacf4b6ae81ea597a43c90c274194a08738f6e760b596&o=&hp=1", // Original IMAGE_3
  "https://cf.bstatic.com/xdata/images/hotel/max1280x900/84555265.jpg?k=ce7c3c699dc591b8fbac1a329b5f57247cfa4d13f809c718069f948a4df78b54&o=&hp=1", // Original IMAGE_4
  ...
  "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHBC04osXmko6Gkli-uPXgZReM25KmjIAZhA&s", 
  ...
  "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8GpZ1hSLfYYcM-txTowyJ-nfUMfSRq5Nxcg&s",
  "https://image-tc.galaxy.tf/wijpeg-28avg4se3pyr83ay0ydwweew6/grand-park-city-hall-singapore_standard.jpg?crop=106%2C0%2C1708%2C1281"
];

// Function to get a random subset of images
const getRandomImages = (min: number, max: number): string[] => {
...
const seedDatabase = async () => {
  try {
    ...
    await Amenity.deleteMany({}); // Clear Amenity data

    console.log("Cleared existing data (Hotels, Locations, Amenities)");
    ...
    // Insert amenities
    const amenityDocs = AMENITY_NAMES.map((name) => ({ name }));
    const createdAmenities = await Amenity.insertMany(amenityDocs);
    console.log(`Created ${createdAmenities.length} amenities`);

    // Get an array of Amenity IDs for random assignment
    const amenityIds = createdAmenities.map((a) => a._id);

    // Function to get a random subset of Amenity IDs
    const getRandomAmenityIds = (min: number, max: number) => {
      const count = Math.floor(Math.random() * (max - min + 1)) + min;
    ...
    // Insert hotels
    // Map original hotels array to a new array with the additional embedding field, images, and amenities
    ...
    const hotelsWithEmbedding = hotels.map(async (hotel) => {
      ...
      const images = getRandomImages(3, 6); // Assign 3 to 6 random images
      const amenities = getRandomAmenityIds(4, 8); // Assign 4 to 8 random amenities

      return { ...hotel, images, amenities, embedding };
    });
